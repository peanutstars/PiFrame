...
static unsigned int ba_order(unsigned int len, int page_size)
{
	int i;
...
struct BuddyAllocator *ba_create(int size, int page_size)
{
	...
	ba->bitmap = (struct bitmap *)ALLOC(sizeof(struct bitmap) * ba->nr_pages);
	memset(ba->bitmap, 0, sizeof(struct bitmap) * ba->nr_pages);

	for(index = 0, i = sizeof(unsigned int) * 8 - 1; i >= 0; i--) {
		if(ba->nr_pages & (1 << i)) {
			BA_ORDER(ba, index) = i;
...
int ba_alloc(struct BuddyAllocator *ba, int len)
{
	...
	int end = ba->nr_pages;
	int best = -1;
	unsigned int order = ba_order(len, ba->page_size);
